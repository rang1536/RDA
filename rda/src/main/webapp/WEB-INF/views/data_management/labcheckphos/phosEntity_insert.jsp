...
function insert(){
 ...
 console.log(quresearch);
		$.ajax({
			url : "${pageContext.request.contextPath }/ajax/phosEntityInsertDo",
			type : "POST",
			data : quresearch,
			dataType : "json",
			success : function(data) {
				...
				if(result > 0){
					alert('성공적으로 입력하였습니다.');
					location.href="phosEntityListDetail?farmId=${farmId}&entity_id=${entity_id}";
					// window.close();
				}else{
					alert('입력에 실패하였습니다.');
				}
...
<input type="hidden" id="ENTITY_ID" name="ENTITY_ID" value="${entity_id}">
	...
	<div align="left" style="padding-bottom: 5px">
		<font style="color:#000000; font-family:gulim; font-size:14px;"><b><img src="/images/icon/board.png" style="width:20px; position:relative; top:0.2em"> P H O S 검 사 정 보 입 력</b></font>
	</div>
	<table class="table_003 w100p">
		...
		<tr>
			<th>혈액검사 PHOS 채취일</th>
			<td style="padding-left: 5px">
<!-- 				<input name="phos_gat1" id="phos_gat1" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 9999) { this.value = this.value.substring(0,4); } }" value="" type="text" maxlength="4" style="width:50px; text-align: right;">년 -->
<!--             	<input name="phos_gat2" id="phos_gat2" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 12) { alert('1~12 사이의 숫자로 입력바랍니다.'); } }" value="" type="text" maxlength="2" style="width:50px; text-align: right;">월 -->
<!--             	<input name="phos_gat3" id="phos_gat3" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 31) { alert('1~32 사이의 숫자로 입력바랍니다.'); } }" value="" type="text" maxlength="2" style="width:50px; text-align: right;">일 -->
<!--             	<input name="phos_gat4" id="phos_gat4" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 24) { alert('1~24 사이의 숫자로 입력바랍니다.'); } }" value="" type="text" maxlength="2" style="width:50px; text-align: right;">시 -->
...
<select name="tdate" id="tdate">
    ...
    <option value="24">24시</option>
		...
		<tr>
			<th>PHOS</th>
			<td style="padding-left: 5px">
				<input type="text" name="PHOS" id="PHOS" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">  미정<br>
			</td>
		</tr>
		<tr>
			<th>측정 장비</th>
