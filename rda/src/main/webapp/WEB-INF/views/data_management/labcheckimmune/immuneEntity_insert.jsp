...
function insert(){
 ...
 console.log(quresearch);
		$.ajax({
			url : "${pageContext.request.contextPath }/ajax/immuneEntityInsertDo",
			type : "POST",
			data : quresearch,
			dataType : "json",
			success : function(data) {
				...
				if(result > 0){
					alert('성공적으로 입력하였습니다.');
					location.href="immuneEntityListDetail?farmId=${farmId}&entity_id=${entity_id}";
			//		 window.close();
				}else{
					alert('입력에 실패하였습니다.');
				}
...
<select name="tdate" id="tdate">
    ...
    <option value="24">24시</option>
		...
		<tr>
			...
			<td style="padding-left: 5px">
				<input type="text" name="SAA" id="SAA" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">&nbsp;&nbsp;미정<br>
			</td>
		</tr>
		<tr>
			<th>Haptoglobin (Hp)</th>
			<td style="padding-left: 5px">
				<input type="text" name="HAPTOGLOBIN" id="HAPTOGLOBIN" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">Hp&nbsp;&nbsp;(0.00~0.05)<br>
			</td>
		</tr>
		<tr>
			<th>CRP (C-Reactive Protein)</th>
			<td style="padding-left: 5px">
				<input type="text" name="CRP" id="CRP" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">&nbsp;&nbsp;미정<br>
			</td>
		</tr>
		<tr>
			<th>IgG (mg/dl)</th>
			<td style="padding-left: 5px">
				<input type="text" name="IGG" id="IGG" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">mg/dl&nbsp;&nbsp;(1700~2700)<br>
			</td>
		</tr>
		<tr>
			<th>IgA (mg/dl)</th>
			<td style="padding-left: 5px">
				<input type="text" name="IGA" id="IGA" onkeyup="if ( isNaN(this.value) ) { alert('숫자만 입력가능합니다.'); this.value=''; } else { if(this.value > 999) { this.value = this.value.substring(0,4); } }" value="">mg/dl&nbsp;&nbsp;(6~100)<br>
			</td>
		</tr>
				<tr>
			<th>측정 장비</th>
